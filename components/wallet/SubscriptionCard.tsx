  ...
  return (
    <div
      className={clsx(
        ...
        highlighted ? "border-gold bg-gold-soft/40" : "border-border bg-surface"
      )}
    >
      <div className="flex items-center justify-between">
        <p className="text-[15px] font-semibold text-text">{plan.name}</p>
        {highlighted && (
          <span className="rounded-full bg-gold px-2 py-0.5 text-[10px] font-semibold text-[rgb(20_16_8)]">
            Best value
          </span>
        )}
      </div>
      ...
      <ul className="mt-3 space-y-1.5 text-[13px] text-text/85">
        <li className="flex items-center gap-1.5">
          <Check size={14} className="text-gold" /> Unlimited back-catalog
        </li>
        <li className="flex items-center gap-1.5">
          <Check size={14} className="text-gold" /> Ad-free
        </li>
        {plan.includes_new_releases && (
          <li className="flex items-center gap-1.5">
            <Check size={14} className="text-gold" /> Day-one new releases
          </li>
        )}
      </ul>
      <Button
